...
import { SuaBangGiaDienService } from '../../../ApiServices/HoaDonAPI/SuaBangGiaDien';
...
const SuaBangGiaDien = ({ dataSua, success, GetHoaDonByIDSVRefetch, setToggle }) => {
  ...
  useEffect(() => {
    if (SuaBangGiaDienResponse) {
      if (SuaBangGiaDienResponse.message === "Sửa bảng giá điện thành công") {
        setToggle(false);
  ...
  return (
    <div className='modal'>
      ...
      <Form
        ...
        autoComplete="off"
      >
        <h2 style={{ textAlign: 'center' }}>Sửa bảng giá điện</h2>
        <Form.Item
          label="Giá điện mới"
          name="price"
          rules={[

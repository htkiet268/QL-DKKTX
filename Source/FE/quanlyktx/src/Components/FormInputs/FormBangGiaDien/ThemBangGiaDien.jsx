...
import { ThemBangGiaDienService } from '../../../ApiServices/HoaDonAPI/ThemBangGiaDien';
...
const ThemBangGiaDien = ({ success, GetHoaDonByIDSVRefetch, setToggle }) => {
  ...
  useEffect(() => {
    if (ThemBangGiaDienResponse) {
      if (ThemBangGiaDienResponse.message === "Thêm mới bảng giá điện thành công") {
  ...
  return (
    <div className='modal'>
      ...
      <Form
        ...
        autoComplete="off"
      >
        <h2 style={{ textAlign: 'center' }}>Thêm bảng giá điện</h2>
        <Form.Item
          label="Giá điện mới"
          name="price"
          rules={[
            {
              required: true,
              message: 'Vui lòng nhập giá điện mới!',
            },
          ]}
        >
          <Input type='number' />

...
import { XoaBangGiaService } from '../../ApiServices/HoaDonAPI/XoaBangGiaNuoc';

import ThemBangGiaDien from '../FormInputs/FormBangGiaNuoc/ThemBangGiaDien';
import SuaBangGiaDien from '../FormInputs/FormBangGiaNuoc/SuaBangGiaDien';
import { FormXacNhan } from '../FormXacNhan/FormXacNhan';

export const HoaDonNuoc = () => {
  ...
  useEffect(() => {
    if (XoaBangGiaResponse) {
      if (XoaBangGiaResponse.message === "Xóa bảng giá nước thành công") {
        setToggleXoa(false);
  ...
  return (
    <div style={{ position: 'relative' }}>
      {contextHolder}
      <TableCustom handleOnclickThem={HandleThem} title={'Bảng giá nước mới'} columns={columns} data={GetAllGetAllBangGiaDienResponse ? GetAllGetAllBangGiaDienResponse : ''} />
      {
      ...
      {
        toggleXoa ? <FormXacNhan message='Bạn chắc chắn xóa Giá nước này chứ?' setToggle={setToggleXoa} action={actionXoa} /> : ''
      }
    </div>
  )
}
